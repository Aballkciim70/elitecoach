...
(() => {
  ...
  const TABLE_TO_KEY = {
    ...
    bookings: "bookings",
    // 'profiles' est volontairement EXCLU : c'est la table gérée par
    // l'authentification (ensureProfile / upsert au login). On ne
    // veut jamais intercepter ses écritures, même en lecture seule
    // de risque. Elle reste chargée en cache via SOURCES.profile,
    // simplement jamais ré-écrite par le hook universel.
  };
  ...
  function installWriteHook() {
    ...
    c.from = function (table) {
      ...
      return builder;
      } catch (e) { console.warn("KpsulStore hook (repli sans danger)", e); return originalFrom(table); }
    };
    } catch (e) { console.warn("KpsulStore installWriteHook (désactivé, sans danger)", e); }
  }
