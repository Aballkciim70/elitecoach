...
(() => {
      ...
      panel.innerHTML = `
        <div class="tool-panel">
          ...
          <p style="color:#8A9A93;font-size:13px;margin:0 0 14px;line-height:1.5">
            ...
            séances, repas, sommeil, mesures. Plus tu renseignes, plus il devient fiable.
          </p>
          <div class="ks3-stats">
  ...
  function compute(D) {
    ...
    R.cardio = { measured:false, hint:"Pas encore mesurable — le suivi cardio arrive dans une prochaine version." };
    ...
    /* Global pondéré sur les métriques mesurées + fiabilité */
    let wSum = 0, sSum = 0, wMeasured = 0;
    METRICS.forEach(m => {
      const r = R[m.key];
      if (r?.measured) { wSum += m.weight; sSum += r.score * m.weight; wMeasured += m.weight; }
    });
    const global = wSum ? round(sSum / wSum) : 0;
    const confidence = round(wMeasured); // les poids totalisent 100
  ...
  function render(results, D) {
    ...
    const level = confidence >= 70 ? "Fiable" : confidence >= 40 ? "Moyennement fiable" : "Peu fiable";
    const conf = $("ks3Conf");
    if (conf) conf.innerHTML = `${level} · <b>${confidence} %</b> · ${nMeasured}/${METRICS.length} mesures`;
