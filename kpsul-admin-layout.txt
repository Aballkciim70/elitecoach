...
(() => {
  ...
  function ensureZone() {
    ...
    // Porte de sortie : bouton retour en tête du tableau de bord,
    // visible même avant la validation 2FA (jamais verrouillé).
    if (!$("viewClientSiteBtn")) {
      const wrap = document.createElement("div");
      wrap.id = "kalReturnWrap";
      wrap.style.margin = "0 0 14px";
      const back = document.createElement("button");
      back.id = "viewClientSiteBtn";
      back.type = "button";
      back.className = "kal-acc-close";
      back.textContent = "← Retour au site client";
      back.addEventListener("click", () => {
        // Si le gouverneur de contexte est absent, bascule legacy.
        if (!window.KpsulContext) document.body.classList.remove("admin-mode");
      });
    ...
    Array.from(dash.children).forEach(ch => {
      if (!["kpsulCoachZone","kalReturnWrap"].includes(ch.id)) ch.classList.add("kal-locked");
    });
    return zone;
  }
  ...
  function relocate() {
    ...
    const mirror = card => {
      ...
      if (!goto || !grid) return;
      if (grid.querySelector(`[data-kal-goto="${goto}"]`)) return;
