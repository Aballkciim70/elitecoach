...
(() => {
  ...
  async function readIdentity() {
    ...
    // Par sécurité, CHAQUE chargement démarre dans le profil client.
    // L'administration ne s'ouvre que sur action explicite (bouton),
    // jamais restaurée automatiquement : impossible de rester enfermé.
    state.activeWorkspace = "client";
  ...
  function bindNavigationCapture() {
    document.addEventListener("click", event => {
      // La déconnexion passe TOUJOURS, avant toute autre logique.
      if (event.target.closest?.("#logoutBtn, [data-logout], a[href*='logout']")) return;
