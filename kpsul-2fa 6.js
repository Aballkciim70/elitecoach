...
(() => {
  ...
  const PENDING_KEY = "kpsul2fa_pending"; // survit aux rechargements iOS

  // Triple persistance : si iOS efface l'un, les autres restent.
  function savePending(obj) {
    const v = JSON.stringify(obj);
    try { localStorage.setItem(PENDING_KEY, v); } catch (_) {}
    try { sessionStorage.setItem(PENDING_KEY, v); } catch (_) {}
    try { document.cookie = PENDING_KEY + "=" + encodeURIComponent(v) + ";path=/;max-age=1800;SameSite=Lax"; } catch (_) {}
  }
  function loadPending() {
    for (const store of [localStorage, sessionStorage]) {
      try { const v = store.getItem(PENDING_KEY); if (v) return JSON.parse(v); } catch (_) {}
    }
    try {
      const m = document.cookie.match(new RegExp(PENDING_KEY + "=([^;]+)"));
  ...
  function clearPending() {
    ...
    try { document.cookie = PENDING_KEY + "=;path=/;max-age=0"; } catch (_) {}
  }
  ...
  function injectStyle() {
    ...
    st.textContent = `
      ...
      .k2fa-steps-box p { color:#C6D0CB; font-size:14px; line-height:1.5 }
      .k2fa-steps-box b { color:var(--paper,#ECEFE9) }
  ...
  async function refreshState() {
    ...
    if (!factorId) {
      // Une inscription était-elle en cours avant un rechargement ?
      const pending = loadPending();
      const alive = pending && (factors?.totp || [])
        .some(f => f.id === pending.id && f.status !== "verified");
      if (alive) { enrollingId = pending.id; return { state: "pending" }; }
      if (pending) clearPending();
      return { state: "enroll" };                          // pas encore de 2FA
    }
  ...
  async function applyState() {
    ...
    const unlocked = state === "unlocked" ||
      (isCoach && (state === "enroll" || state === "pending"));
  ...
  function renderPanel(state) {
    ...
    if (state === "pending") {
      box.innerHTML = `
        <div class="k2fa-box">
          <p><b>Finalise ton activation.</b> La page s'est rechargée, mais ton inscription est
          conservée : le compte est déjà (ou presque) dans ton app d'authentification.
          Entre le code à 6 chiffres qu'elle affiche :</p>
          ...
          <div class="k2fa-status" id="k2faStatus"></div>
          <div class="k2fa-danger" style="margin-top:14px">
            <button id="k2faRestart" type="button">Recommencer avec un nouveau QR</button>
          </div>
        </div>`;
      ...
      $("k2faRestart")?.addEventListener("click", () => { clearPending(); startEnroll(); });
      setTimeout(() => $("k2faEnrollCode")?.focus(), 120);
      return;
    }
  ...
  async function startEnroll() {
    ...
    // Nettoyage des facteurs fantômes : inscriptions jamais confirmées
    // (elles bloquent la création avec « already exists »).
    try {
      const { data: lf } = await client.auth.mfa.listFactors();
      const ghosts = (lf?.totp || []).filter(f => f.status !== "verified");
      for (const g of ghosts) {
        await client.auth.mfa.unenroll({ factorId: g.id });
      }
    } catch (_) { /* au pire, l'enroll échouera avec un message clair */ }

    const { data, error } = await client.auth.mfa.enroll({
      factorType: "totp",
      friendlyName: "kpsul-coach-" + Date.now().toString(36)
    });
    if (error) { setStatus("Erreur : " + error.message, true); return; }
    enrollingId = data.id;
    savePending({ id: data.id, at: Date.now() });

    const zone = $("k2faEnrollZone");
    if (zone) {
      zone.innerHTML = `
        <div class="k2fa-steps-box">
          <p style="margin:0 0 10px"><b>Étape 1.</b> Copie cette clé :</p>
          <div class="k2fa-secret" id="k2faSecret">${safe(data.totp.secret)}</div>
          <button class="btn btn-ghost" id="k2faCopy" type="button" style="width:100%;margin-bottom:6px">📋 Copier la clé</button>

          <p style="margin:14px 0 6px"><b>Étape 2.</b> Ouvre ton app d'authentification,
          choisis « saisir une clé de configuration » (ou « + » → « Saisie manuelle »),
          colle la clé, et reviens ici. <b style="color:var(--core,#34E0C8)">Tu peux fermer et rouvrir Kpsul sans problème</b> — ton activation est sauvegardée.</p>

          <details style="margin:8px 0 12px">
            <summary style="color:#8A9A93;font-size:13px;cursor:pointer">Ou scanner un QR (si 2e appareil)</summary>
            <img class="k2fa-qr" src="${data.totp.qr_code}" alt="QR code 2FA" style="margin-top:10px">
            <a class="btn btn-ghost" style="display:block;text-align:center" href="${data.totp.uri}">📲 Ouvrir dans mon app</a>
          </details>

          <p style="margin:14px 0 6px"><b>Étape 3.</b> Entre le code à 6 chiffres affiché par ton app :</p>
          <input class="k2fa-code-input" id="k2faEnrollCode" inputmode="numeric" maxlength="6" placeholder="••••••" autocomplete="one-time-code">
          <button class="btn btn-primary" id="k2faConfirmBtn" type="button" style="width:100%;margin-top:12px">Activer la 2FA</button>
        </div>`;
      $("k2faCopy")?.addEventListener("click", async () => {
        const key = data.totp.secret;
        try { await navigator.clipboard.writeText(key); setStatus("✔ Clé copiée."); }
        catch (_) {
          // Repli : sélectionner le texte pour copie manuelle
          const el = $("k2faSecret");
          if (el) { const r = document.createRange(); r.selectNode(el);
            getSelection().removeAllRanges(); getSelection().addRange(r); }
          setStatus("Sélectionne la clé et copie-la.", false);
        }
      });
      $("k2faConfirmBtn")?.addEventListener("click", confirmEnroll);
      $("k2faEnrollCode")?.addEventListener("input", e => {
    ...
    setStatus("Copie la clé, ajoute-la à ton app, puis entre le code.");
  }
