...
(() => {
  ...
  function relocate() {
    ...
    // b) Modules tiers RÉSERVÉS AU COACH mais injectés sans admin-zone :
    //    coach -> rapatriés dans l'admin ; client -> supprimés (il ne doit pas les voir)
    qsa('#member [data-goto="modCoachCrm"]').forEach(card =>
      isCoachBody ? toGrid(card) : card.remove());

    // c) Modules BI-MODES (contenu coach quand on est coach) : déplacés côté admin uniquement
    if (isCoachBody) {
      qsa('#member [data-goto="modAlerts"]').forEach(toGrid);
    }
