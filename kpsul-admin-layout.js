...
(() => {
  ...
  function relocate() {
    ...
    const isCoachBody = document.body.classList.contains("admin");
    const grid = $("kalToolsGrid");

    // Clone visible dans l'admin + ORIGINAL MASQUÉ sur place :
    // le module d'origine retrouve sa carte -> plus de réinjection en boucle.
    const mirror = card => {
      const goto = card.dataset.goto || card.dataset.kalGoto || "";
      card.style.setProperty("display", "none", "important");
      card.dataset.kalHidden = "1";
      if (!goto || !grid) return;
      if (grid.querySelector(`[data-kal-goto="${goto}"]`)) return; // déjà cloné
      const clone = card.cloneNode(true);
      clone.style.removeProperty("display");
      delete clone.dataset.kalHidden;
      clone.dataset.kalGoto = goto;
      clone.removeAttribute("data-goto");
      grid.appendChild(clone);
    };

    // a) Cartes suivant la convention admin-zone
    qsa("#member .mtile.admin-zone, #member .kpsul-lock-card").forEach(card => {
      if (card.classList.contains("kpsul-lock-card") ||
          ["modKpsulCRM","modKpsul2FA","modAdmin"].includes(card.dataset.goto)) {
        card.remove();
        return;
      }
      mirror(card);
    ...
    // b) Modules tiers réservés au coach, injectés sans admin-zone
    qsa('#member [data-goto="modCoachCrm"]:not([data-kal-hidden])').forEach(card => {
      if (isCoachBody) mirror(card);
      else card.style.setProperty("display", "none", "important"); // client : jamais visible
    });

    // c) Modules bi-modes : côté coach, la carte vit dans l'admin
    if (isCoachBody) {
      qsa('#member [data-goto="modAlerts"]:not([data-kal-hidden])').forEach(mirror);
    }

    // Dédoublonnage curatif de la grille (répare les doublons déjà créés)
    if (grid) {
      qsa('[data-kal-goto="modAdmin"]', grid).forEach(c => c.remove());
      const seen = new Set();
      qsa("[data-kal-goto]", grid).forEach(c => {
        const g = c.dataset.kalGoto;
        if (seen.has(g)) c.remove(); else seen.add(g);
      });
    }

    if (grid?.children.length) $("kpsulSlotTools").style.display = "";
  }
  ...
  function hookToolsGrid() {
    $("kalToolsGrid")?.addEventListener("click", e => {
      ...
      const pl = acc.querySelector(".module-panel") || panel;
      pl.hidden = false; pl.inert = false; pl.removeAttribute("aria-hidden");
      // Charger les données du module ouvert (sans passer par sa navigation membre)
      if (id === "modAlerts") window.KpsulAlerts?.refresh?.();
  ...
  window.addEventListener("kpsul:context-change", e => {
    if (e.detail?.isAdminWorkspace) setTimeout(onEnterAdmin, 120);
  });
  ...
  const tilesObs = new MutationObserver(() => {
    if (document.querySelector('#member .mtile.admin-zone, #member .kpsul-lock-card, #member [data-goto="modCoachCrm"], #member [data-goto="modAlerts"]')) relocate();
  });
  function watchTiles() {
    const g = grid();
