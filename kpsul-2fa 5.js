...
(() => {
  ...
  async function refreshState() {
    ...
    if (!factorId) {
      ...
      if (pending) { try { localStorage.removeItem(PENDING_KEY); } catch (_) {} }
  ...
  function renderPanel(state) {
    ...
    if (state === "pending") {
      ...
      $("k2faRestart")?.addEventListener("click", () => {
  ...
  async function startEnroll() {
    ...
    enrollingId = data.id;
    try { localStorage.setItem(PENDING_KEY, JSON.stringify({ id: data.id, at: Date.now() })); } catch (_) {}

    const zone = $("k2faEnrollZone");
    if (zone) {
      zone.innerHTML = `
        <a class="btn btn-primary" style="display:block;text-align:center;margin:14px 0 6px"
           href="${data.totp.uri}">📲 Ajouter à mon app en un tap</a>
        <p style="text-align:center;color:#8A9A93;font-size:12px;margin:0 0 4px">
          Recommandé sur iPhone : choisis l'app <b style="color:#C6D0CB">Mots de passe</b> d'Apple —
          tes codes s'afficheront ensuite au-dessus du clavier, sans quitter le site.
          Si la page se recharge en revenant, l'activation reprend où tu en étais.</p>
