  ...
  async function applyState() {
    ...
    applyPromise = (async () => {
      ...
      const shouldUnlock =
        state === "unlocked" ||
        state === "notcoach" ||
        (isCoach && state === "enroll");

      if (document.body.classList.contains("kpsul-aal2") !== shouldUnlock) {
        document.body.classList.toggle("kpsul-aal2", shouldUnlock);
      }
...
/* ─── INSCRIPTION 2FA ──────────────────────────────────────── */
  ...
  async function startEnroll() {
    ...
    // Ouvre immédiatement l'onglet pendant le clic utilisateur.
    // Cela évite le blocage des pop-ups sur Safari/iPhone.
    const qrWindow = window.open("", "_blank");

    if (!qrWindow) {
      setStatus("Autorise les fenêtres surgissantes pour afficher le QR code.", true);
      return;
    }
    ...
    qrWindow.document.write(`<!doctype html>
      ...
      <head>
        ...
        <title>QR code KPSUL</title>
        <style>
          *{box-sizing:border-box}
          body{
            margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;
            padding:24px;background:#07110f;color:#f4f7f6;
            font-family:system-ui,-apple-system,Arial,sans-serif;text-align:center
          }
          .box{width:min(100%,420px)}
          .loader{font-size:18px;color:#34e0c8}
        </style>
      </head>
      <body>
        <div class="box"><div class="loader">Génération du QR code…</div></div>
      </body>
      </html>`);
    ...
    try {
      ...
      if (!qrSource) throw new Error("Le QR code reçu est vide.");

      // Affichage dans la page extérieure.
      qrWindow.document.open();
      qrWindow.document.write(`<!doctype html>
        <html lang="fr">
        <head>
          ...
          <style>
            *{box-sizing:border-box}
            ...
            .box{
              width:min(100%,430px);padding:24px;border:1px solid #22403a;
              border-radius:24px;background:#0d1916
            }
            h1{margin:0 0 8px;font-size:24px}
            p{color:#aab7b1;line-height:1.5}
            img{
              display:block;width:280px;height:280px;max-width:100%;
              margin:22px auto;background:#fff;padding:12px;border-radius:18px;
              object-fit:contain
            }
            .secret{
              margin-top:14px;padding:12px;border:1px dashed #31544c;
              border-radius:12px;word-break:break-all;color:#34e0c8;
        ...
        <body>
          <main class="box">
            <h1>QR code KPSUL</h1>
            <p>Scanne ce QR code avec Google Authenticator, Authy ou l’app Mots de passe d’Apple.</p>
            <img src="${safe(qrSource)}" alt="QR code 2FA KPSUL">
            <p class="note">Si le scan ne fonctionne pas, utilise cette clé manuelle :</p>
            <div class="secret">${safe(secret)}</div>
            <p class="note">Reviens ensuite sur KPSUL pour saisir le code à 6 chiffres.</p>
          </main>
        </body>
        </html>`);
      qrWindow.document.close();

      // Dans KPSUL, on affiche uniquement la saisie du code.
      if (zone) {
        zone.innerHTML = `
          <div style="margin-top:16px">
            <p style="text-align:center;color:#8A9A93;font-size:13px">
              Le QR code s’est ouvert dans un nouvel onglet.
            </p>
            <input class="k2fa-code-input" id="k2faEnrollCode"
              inputmode="numeric" pattern="[0-9]*" maxlength="6"
              placeholder="••••••" autocomplete="one-time-code">
            <button class="btn btn-primary" id="k2faConfirmBtn"
              type="button" style="width:100%;margin-top:12px">
              Confirmer le code
            </button>
          </div>`;
      ...
      setStatus("QR ouvert. Entre ensuite le code à 6 chiffres.");
      ...
      try {
        ...
        qrWindow.document.write(`<!doctype html>
          ...
          <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width,initial-scale=1">
            <title>Erreur QR KPSUL</title>
            <style>
              body{
                margin:0;min-height:100vh;display:flex;align-items:center;
                justify-content:center;padding:24px;background:#07110f;
                color:#f4f7f6;font-family:system-ui;text-align:center
              }
              div{max-width:420px}
              h1{color:#e8735b}
            </style>
          ...
          <body>
            <div>
              <h1>QR code impossible à générer</h1>
      ...
      setStatus(
        "Erreur : " + (error?.message || "impossible de générer le QR code."),
        true
      );
    } finally {
  ...
  async function confirmEnroll() {
    ...
    setTimeout(() => {
      applyState().catch(error => console.error("Kpsul2FA enroll verified", error));
    }, 300);
  }
  ...
  async function verifyChallenge() {
    ...
    setTimeout(() => {
      applyState().catch(error => console.error("Kpsul2FA unlock", error));
    }, 300);
  }
...
function setStatus(msg, err = false) {
  ...
  function installAuthListener() {
    ...
    client.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_OUT" || !session) {
        ...
        enrollingId = null;
        isCoach = false;
        document.body.classList.remove("kpsul-aal2");
        return;
      }
  ...
  function init() {
    ...
    initPromise
      .catch(error => console.error("Kpsul2FA init", error))
  ...
  new MutationObserver(() => {
    ...
    if (authed === lastAuthedState) return;
    lastAuthedState = authed;
  ...
  lastAuthedState = document.body.classList.contains("authed");
  if (lastAuthedState) init();
})();
