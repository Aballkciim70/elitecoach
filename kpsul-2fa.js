...
(() => {
  ...
  async function applyState() {
    // Verrouillage immédiat pendant la vérification de la session.
    ...
    // Un coach sans 2FA configurée garde l'accès pour pouvoir l'activer.
    if (isCoach && state === "enroll") {
...
function setStatus(msg, err = false) {
    ...
    client.auth.onAuthStateChange((event, session) => {
      ...
      if (
        ...
        setTimeout(() => {
          ...
          injectCard();
          injectModule();
          applyState().catch(error =>
            console.error("Kpsul2FA auth state", error)
          );
        }, 100);
      }
    });
  ...
  function init() {
    ...
    injectModule();
    installAuthListener();
    applyState().catch(error => console.error("Kpsul2FA init", error));
  }
